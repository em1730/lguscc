...
$sql = "SELECT docno, date, obrno, dvno, type, payee, particulars, amount, origin";
$sql.=" FROM tbl_documents where status in ('CREATED', 'FORWARDED') and destination = '$office' ORDER BY date DESC LIMIT ".$requestData['start']." ,".$requestData['length']."  "; ;
...
$sql.=" FROM tbl_documents where status in ('CREATED', 'FORWARDED') and destination = '$office'";
